...
def plot_data_2():
    ...
    ax2.set_xlabel("Date et heure (heure anglaise)")
    ...
    # Archive the plot
    # plt.savefig(plot_archive_file)
    # print(f"Graphique enregistré dans {PLOT_FILE} et archivé dans {plot_archive_file}")
    plt.close()


# Exécution
count_and_plot = True # Si True, on compte les signatures et on trace le graphique. Si False, on ne fait que tracer le graphique (utile pour les tests).
...
if count_and_plot:
    ...
    if count is not None:
        ...
        print("Erreur : impossible de récupérer le nombre de signatures.")
else:
    plot_data_2()